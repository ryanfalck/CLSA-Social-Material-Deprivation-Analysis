...
Group1<-subset(wide.data2, Agebin.BL=='45-64' & SEX_ASK.BL=='M')
Group2<-subset(wide.data2, Agebin.BL=='65+' & SEX_ASK.BL=='F')
Group3<-subset(wide.data2, Agebin.BL=='45-64' & SEX_ASK.BL=='F')
Group4<-subset(wide.data2, Agebin.BL=='65+' & SEX_ASK.BL=='M')

sd2a <-sd1*sd(Group1$PASE_total.BL, na.rm=TRUE)/100
sd2b <-sd1*sd(Group2$PASE_total.BL, na.rm=TRUE)/100
sd2c <-sd1*sd(Group3$PASE_total.BL, na.rm=TRUE)/100
sd2d <-sd1*sd(Group4$PASE_total.BL, na.rm=TRUE)/100

sd3a <-sd1*sd(Group1$Material_deprivation.BL, na.rm=TRUE)
sd3b <-sd1*sd(Group2$Material_deprivation.BL, na.rm=TRUE)
sd3c <-sd1*sd(Group3$Material_deprivation.BL, na.rm=TRUE)
sd3d <-sd1*sd(Group4$Material_deprivation.BL, na.rm=TRUE)

sd4a <-sd1*sd(Group1$Social_deprivation.BL, na.rm=TRUE)
sd4b <-sd1*sd(Group2$Social_deprivation.BL, na.rm=TRUE)
sd4c <-sd1*sd(Group3$Social_deprivation.BL, na.rm=TRUE)
sd4d <-sd1*sd(Group4$Social_deprivation.BL, na.rm=TRUE)
```
...
EF_PA2a$Group<-"Females 65+ years"
...
EF_PA3a$Group<-"Females 45-64 years"
...
EF_PA6a$Group<-"Females 65+ years"
...
Mem_PA2a$Group<-"Females 65+ years"
...
Mem_PA3a$Group<-"Females 45-64 years"
...
Mem_PA6a$Group<-"Females 65+ years"
...
EF_Mat6a$Group<-"Females 65+ years"

Mat2<-EF_Delta_Mat_b7*(sd2c)
...
EF_Mat7a$Group<-"Females 45-64 years"

Mat2<-EF_Delta_Mat_b8*(sd2c)
...
Mem_Mat2a$Group<-"Females 65+ years"

Mat1<-Mem_BL_Mat_b7*(sd2c)
...
Mem_Mat3a$Group<-"Females 45-64 years"

Mat1<-Mem_BL_Mat_b8*(sd2d)
...
Mem_Mat4a$Group<-"Males 65+ years"
...
Mem_Mat6a$Group<-"Females 65+ years"

Mat2<-Mem_Delta_Mat_b7*(sd2c)
...
Mem_Mat7a$Group<-"Females 45-64 years"

Mat2<-Mem_Delta_Mat_b8*(sd2c)
...
Mem_Mat8a$Group<-"Males 65+ years"
...
EF_Soc6a$Group<-"Females 65+ years"

Soc2<-EF_Delta_Soc_b7*(sd2c)
...
EF_Soc7a$Group<-"Females 45-64 years"

Soc2<-EF_Delta_Soc_b8*(sd2c)
...
Mem_Soc2a$Group<-"Females 65+ years"

Soc1<-Mem_BL_Soc_b7*(sd2c)
...
Mem_Soc3a$Group<-"Females 45-64 years"

Soc1<-Mem_BL_Soc_b8*(sd2d)
...
Mem_Soc4a$Group<-"Males 65+ years"
...
Mem_Soc6a$Group<-"Females 65+ years"

Soc2<-Mem_Delta_Soc_b7*(sd2c)
...
Mem_Soc7a$Group<-"Females 45-64 years"

Soc2<-Mem_Delta_Soc_b8*(sd2c)
...
Mem_Soc8a$Group<-"Males 65+ years"
...
```{r}
se1d<-sqrt(EF_Delta_PA_var1 + 2*(sd4a)*EF_Delta_SocINT_cov1 + (sd4a^2)*EF_Delta_SocINT_var1)
...
Moderating effect of baseline social deprivation on the association between physical activity and baseline executive function.
```{r}
se4b<-sqrt(EF_BL_PA_var4 + 2*(sd4d)*EF_BL_SocINT_cov4 + (sd4d^2)*EF_BL_SocINT_var4)

b4b<-EF_BL_PA_b4+EF_BL_SocINT_b4*(sd4d)
...
Note that variance in physical activity was limited for this model, and thus we created a smaller scale in order for standard errors to be fit for analysis
```{r}
sd3<-c(-2, -1.75, -1.5, -1.25, -1, -.75, -.5, -.25, 0,
       .25, 0.3)

sd3c <-sd3*sd(Group3$Material_deprivation.BL, na.rm=TRUE)
...
```{r}
se3g<-sqrt(Mem_Delta_PA_var3 + 2*(sd3c)*Mem_Delta_MatINT_cov3 + (sd3c^2)*Mem_Delta_MatINT_var3)

b3g<-Mem_Delta_PA_b3+Mem_Delta_MatINT_b3*(sd3c)

UB3g<-b3g + se3g*qnorm(.975)
LB3g<-b3g + se3g*qnorm(0.025)

Mem_Delta3a<-data.frame(b3g,sd3,UB3g,LB3g)

ggplot(Mem_Delta3a)+
  geom_ribbon(aes(ymin=LB3g, ymax=UB3g, x=sd3), alpha=.7)+
  geom_line(aes(y=b3g, x=sd3))+
  geom_hline(aes(yintercept=0), lty='dashed') +
  theme_bw() + 
  theme(panel.border = element_blank(), panel.grid.major = element_blank(),
                     panel.grid.minor = element_blank(), axis.line = element_line(colour = "black")) +
...
Moderating effect of baseline PASE score on the association between social deprivation and baseline executive function.
```{r}
se4b<-sqrt(EF_BL_Soc_var4 + 2*(sd2d)*EF_BL_SocINT_cov4 + (sd2d^2)*EF_BL_SocINT_var4)

b4b<-EF_BL_Soc_b4+EF_BL_SocINT_b4*(sd2d)
...
```{r}
se3g<-sqrt(Mem_Delta_Mat_var3 + 2*(sd2c)*Mem_Delta_MatINT_cov3 + (sd2c^2)*Mem_Delta_MatINT_var3)

b3g<-Mem_Delta_Mat_b3+Mem_Delta_MatINT_b3*(sd2c)
